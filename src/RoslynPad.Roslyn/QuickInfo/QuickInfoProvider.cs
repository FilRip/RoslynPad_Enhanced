...
internal sealed class QuickInfoProvider(IDeferredQuickInfoContentProvider contentProvider) : IQuickInfoProvider
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var bestBinding = candidateResults.Find(c => c.Item3.Count > 0 && !ErrorVisitor.ContainsError(c.Item3[0]));

        // Every file binds with errors. Take the first candidate, which is from the current file.
        bestBinding ??= candidateResults[0];
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (sections.TryGetValue(SymbolDescriptionGroups.TypeParameterMap, out var parts) && !parts.IsDefaultOrEmpty)
        {
        ...
        if (sections.TryGetValue(SymbolDescriptionGroups.StructuralTypes, out parts) && !parts.IsDefaultOrEmpty)
        {
        ...
        if (sections.TryGetValue(SymbolDescriptionGroups.AwaitableUsageText, out parts) && !parts.IsDefaultOrEmpty)
        {
        ...
        if (sections.TryGetValue(SymbolDescriptionGroups.Exceptions, out parts) && !parts.IsDefaultOrEmpty)
        {
    ...
    private static async Task<ValueTuple<SemanticModel, IList<ISymbol>>> BindTokenAsync(
        Document document,
        SyntaxToken token,
        CancellationToken cancellationToken)
    {
    ...
    private sealed class ErrorVisitor : SymbolVisitor<bool>
    {
        ...
        {
            if (symbol.TypeArguments.Concat(symbol.TypeParameters).Any(typeParameter => Visit(typeParameter)))
                return true;
