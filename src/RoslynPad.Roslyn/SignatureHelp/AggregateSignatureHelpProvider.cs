...
internal sealed class AggregateSignatureHelpProvider([ImportMany] IEnumerable<Lazy<Microsoft.CodeAnalysis.SignatureHelp.ISignatureHelpProvider, OrderableLanguageMetadata>> providers) : ISignatureHelpProvider
{
    ...
    public async Task<SignatureHelpItems?> GetItemsAsync(Document document, int position, SignatureHelpTriggerInfo triggerInfo, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var provider in _providers)
        {
            ...
            var currentItems = await provider.GetItemsAsync(document, position, triggerInfo.Inner, SignatureHelpOptions.Default, cancellationToken).ConfigureAwait(false);
            if (currentItems != null && currentItems.ApplicableSpan.IntersectsWith(position) && IsBetter(bestItems, currentItems.ApplicableSpan))
            {
    ...
    private static class DefaultSignatureHelpSelector
    {
            ...
            IList<SignatureHelpItem> filteredItems, int selectedParameter, int argumentCount, string? name, bool isCaseSensitive)
        {
            ...
            var lastItem = filteredItems[filteredItems.Count - 1];
