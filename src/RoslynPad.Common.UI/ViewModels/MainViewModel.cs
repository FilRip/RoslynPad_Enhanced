...
public abstract class MainViewModel : NotificationObject, IDisposable
{
    ...
    protected MainViewModel(IServiceProvider serviceProvider, ITelemetryProvider telemetryProvider, ICommandProvider commands, IApplicationSettings settings, NuGetViewModel nugetViewModel, DocumentFileWatcher documentFileWatcher)
    {
        ...
        OpenFileCommand = commands.CreateAsync(OpenFileAsync);
        CloseCurrentDocumentCommand = commands.CreateAsync(CloseCurrentDocumentAsync);
        CloseDocumentCommand = commands.CreateAsync<OpenDocumentViewModel>(CloseDocumentAsync);
    ...
    public async Task InitializeThemeAsync()
    {
        ...
        (string? path, ThemeType type) GetBuiltinThemePath(BuiltInTheme builtInTheme)
        {
            ...
            {
                bool isSystemDarkTheme = IsSystemDarkTheme();
            ...
            (string file, ThemeType type) = builtInTheme switch
            {
            ...
            return (Path.Combine(AppContext.BaseDirectory, "Themes", file), type);
        }

        async Task LoadThemeAsync(string? themeFile, ThemeType type)
        {
            ...
            ThemeType = type;
            Theme = await _themeManager.ReadThemeAsync(themeFile, type);
        }
    }

    protected abstract void ListenToSystemThemeChanges(Func<Task> onChange);
    ...
    public async Task CloseDocumentAsync(OpenDocumentViewModel? document)
    {
        if (document == null)
        {
            return;
    ...
    public IDelegateCommand SearchCommand => _commands.CreateAsync(SearchAsync);
    ...
    private sealed class DocumentWatcher : IDisposable
    {
